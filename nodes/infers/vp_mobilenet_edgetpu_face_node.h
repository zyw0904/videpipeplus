#pragma once

#include "../vp_primary_infer_node.h"
#include "../../objects/vp_frame_face_target.h"
#include <core/session/onnxruntime_cxx_api.h>
#include <core/session/onnxruntime_c_api.h>

namespace vp_nodes {
    
    extern cv::Mat FRAME;
    
    class vp_mobilenet_edgetpu_face_node: public vp_primary_infer_node
    {
    private:
        // names of output layers in yunet
        float scoreThreshold = 0.85;
        float nmsThreshold = 0.6;
        int topK = 50;
        std::vector<cv::Mat> Mats;
        int inputW;
        int inputH;
        float* detection_boxes;
        float* detection_score;
        float* detection_number;
        //std::vector<std::vector<Point2f>> landmarks;
        int get_color(int c, int classNum);
        std::vector<string> ReadLabels(const string& filename);
        //string GetLabel(const vector<string>& labels, int label);
        
    protected:
        // override infer and preprocess as yunet has a different logic
        //virtual void infer(const cv::Mat& blob_to_infer, std::vector<cv::Mat>& raw_outputs) override;
        //virtual void preprocess(const std::vector<cv::Mat>& mats_to_infer, cv::Mat& blob_to_infer) override;

        //virtual void postprocess(const std::vector<cv::Mat>& raw_outputs, const std::vector<std::shared_ptr<vp_objects::vp_frame_meta>>& frame_meta_with_batch) override;
        
    public:
        vp_mobilenet_edgetpu_face_node(std::string node_name, std::string model_path, std::string convert_type = "", float score_threshold = 0.7, float nms_threshold = 0.6, int top_k = 50);
        ~vp_mobilenet_edgetpu_face_node();
    };
    
}
