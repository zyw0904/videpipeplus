...
namespace vp_nodes {
    ...
    protected:
        ...
        //virtual void infer(const cv::Mat& blob_to_infer, const std::vector<std::vector<int>>& outputs_shape, const std::vector<std::vector<float>>& tensor_vector) override;
    ...
    public:
        vp_yunet_face_detector_node(std::string node_name, std::string model_path, std::string convert_type = "", std::string model_config_path = "", std::string labels_path = "", float score_threshold = 0.7, float nms_threshold = 0.5, int top_k = 50);
